...
var MongoStorageAdapter = exports.MongoStorageAdapter = function () {
  ...
  _createClass(MongoStorageAdapter, [{
    ...
    value: function distinct(className, schema, query, fieldName) {
      schema = convertParseSchemaToMongoSchema(schema);
      return this._adaptiveCollection(className).then(function (collection) {
        return collection.distinct(fieldName, (0, _MongoTransform.transformWhere)(className, query, schema));
      }).then(function (objects) {
        return objects.map(function (object) {
          return (0, _MongoTransform.mongoObjectToParseObject)(className, object, schema);
        });
    ...
    value: function aggregate(className, schema, pipeline, readPreference) {
      ...
      readPreference = this._parseReadPreference(readPreference);
      return this._adaptiveCollection(className).then(function (collection) {
        return collection.aggregate(pipeline, { readPreference: readPreference, maxTimeMS: _this11._maxTimeMS });
